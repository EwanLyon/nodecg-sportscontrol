...
nodecg.listenFor('newTournament', (data: TournamentSetup) => {
	if (!isFixtureType(data.fixtureType)) {
		nodecg.log.warn(
			`Tried to make tournament using fixture type: ${data.fixtureType}. This is not a supported type.`,
		);
		return;
	}

	const fixture: Tournament['fixture'] | undefined = createFixtureData(
		data.fixtureType,
		data.size,
	);
	...
	if (!fixture) {
		nodecg.log.warn(
			`Tried to make tournament data fixture data: ${data}. This is not supported fixture data.`,
		);
		return;
	}
	...
	switch (fixtureType) {
		...
		case 'double-elimination': {
			const doubleFixture: DoubleElimination = {
				...
				loserMatches: [
					...createSingleFixtureArrays((extraData || 0) / 2),
					...createSingleFixtureArrays((extraData || 0) / 2),
...
nodecg.listenFor('setActiveTournament', (id: string) => {
	...
	if (!tournamentsRep.value[id] && id !== '') {
		nodecg.log.warn(
			`Could not set the current tournament to "${id}" as it is not in the database.`,
		);
		return;
	}
...
nodecg.listenFor(
	'setSingleElimMatch',
	(data: { tournamentId: string; matchId: string; round: number; match: number }) => {
		...
		if (isSingleElimination(mutableTournament.fixture)) {
			...
			nodecg.log.warn(
				`Tried to modify a single elimination fixture when the tournament edited is : ${
					tournamentsRep.value[data.tournamentId].fixture.type
				}`,
			);
		}
	},
